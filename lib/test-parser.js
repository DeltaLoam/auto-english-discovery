...
function getMergedGroups(practiceItem) {
  const questions =
  ...
  for (const question of questions) {
    if (Array.isArray(question?.al)) groups.push(...question.al);
  }
  return groups;
}
...
export function buildTaskCandidates(task, practiceItem, maxCandidates = 10000) {
  ...
  const groups = getMergedGroups(practiceItem)
    ?.filter((group) => group?.id !== 0 && group?.id !== undefined)
    .sort((left, right) => Number(left.id) - Number(right.id));
  if (!groups?.length) return [];

  const choices = groups.map((group) => {
    const answers = answerOptions(group);
    if (answers.length > 13) return [];
    return Array.from({ length: 2 ** answers.length - 1 }, (_, mask) =>
      answers.filter((_, index) => (mask + 1 & 2 ** index) !== 0)
    );
  });
  if (choices.some((groupChoices) => groupChoices.length === 0)) return [];

  const candidates = [];
  const visit = (groupIndex, aId) => {
    ...
    if (groupIndex === groups.length) {
      candidates.push({
        ...
        iCode: task.code,
        iType: task.type,
        ua: aId.map((pairs) => ({ qId: "1", aId: pairs })),
      });
      return;
    }

    const group = groups[groupIndex];
    for (const subset of choices[groupIndex]) {
      visit(groupIndex + 1, [
        ...aId,
        subset.map((answer) => [group.id, answer.id]),
      ]);
      if (candidates.length >= maxCandidates) return;
    }
  };
