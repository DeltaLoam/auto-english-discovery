...
import { getTestTasks, parseLessonSource } from "./test-parser.js";
...
export default class engdis {
  ...
  async Login(
    username,
    password,
    InstitutionId = "5232957",
    CommunityVersion = "136",
    cannonicalDomain = "ed22.engdis.com/thai"
  ) {
    try {
      ...
      console.log(`[!] login request failed: ${e.response?.status ?? e.code ?? e.message}`);
      return {
        isSuccess: false,
      };
    }
  ...
  async practiceGetItem(itemId, code, itemType, userTestId = 0, viewMode = 7) {
    try {
      var { data } = await axios.get(
        `/practiceManager/GetItem/${itemId}/${code}/${itemType}/${userTestId}/${viewMode}/`
      );
      return { isSuccess: true, data };
    } catch (e) {
      console.log(`[!] failed to load practice item ${code}: ${e.response?.status ?? e.message}`);
  ...
  async getTestCodeDigit(code) {
    try {
      var { data } = await lessonClient.get(
        `https://static.engdis.com/edprod01/edprod/Runtime/Lessons/${code}.js`
      );
      const lesson = parseLessonSource(data);
      const tasks = getTestTasks(lesson);
      return { isSuccess: true, tasks };
    } catch (err) {
      console.log(`[!] failed to load lesson ${code}: ${err.message}`);
      return {
        isSuccess: false,
        tasks: [],
      };
  ...
  async SaveUserTestV1(nodeId, ParentNodeId, body) {
    try {
      ...
      console.log(`[!] SaveUserTestV1 failed: ${e.response?.data ? JSON.stringify(e.response.data) : e.message}`);
      return { isSuccess: false, data: null };
    }
  }
